...
while ($row = $modx->db->getRow($rs)) {
	...
	switch ($row['type']) {
		...
		//v003
			...
			$tmplvar_arr_prval2	= $_POST["tv".$row['id']."prval2"];
			$tmplvar_arr_prval3	= $_POST["tv".$row['id']."prval3"];
			if( ! empty( $tmplvar_arr_prnm ) )
			{
				foreach( $tmplvar_arr_prnm AS $key => $val )
				{
					...
					$tmplvar_arr_prval2[ $key ]= str_replace( "::", "", trim( $tmplvar_arr_prval2[ $key ] ) );
					$tmplvar_arr_prval2[ $key ]= str_replace( "||", "", $tmplvar_arr_prval2[ $key ] );
					
					$tmplvar_arr_prval3[ $key ]= str_replace( "::", "", trim( $tmplvar_arr_prval3[ $key ] ) );
					$tmplvar_arr_prval3[ $key ]= str_replace( "||", "", $tmplvar_arr_prval3[ $key ] );
					
					if( ! empty( $val ) ) $tmplvar .= ( ! empty( $tmplvar ) ? '||' : '' ) . $val .'::'. $tmplvar_arr_prval[ $key ].'::'. $tmplvar_arr_prval2[ $key ].'::'. $tmplvar_arr_prval3[ $key ];
				}
			}
		break;
		...
		//v003
			...
			$tmplvar_arr_prval2	= $_POST["tv".$row['id']."prval2"];
			$tmplvar_arr_prval3	= $_POST["tv".$row['id']."prval3"];
			$tmplvar_arr_prval4	= $_POST["tv".$row['id']."prval4"];
			if( ! empty( $tmplvar_arr_prnm ) )
			{
				foreach( $tmplvar_arr_prnm AS $key => $val )
				{
					...
					$tmplvar_arr_prval4[ $key ]= str_replace( "::", "", trim( $tmplvar_arr_prval4[ $key ] ) );
					$tmplvar_arr_prval4[ $key ]= str_replace( "||", "", $tmplvar_arr_prval4[ $key ] );
					
					
					if( ! empty( $val ) ) $tmplvar .= ( ! empty( $tmplvar ) ? '||' : '' ) . $val .'::'. $tmplvar_arr_prval[ $key ].'::'. $tmplvar_arr_prval2[ $key ].'::'. $tmplvar_arr_prval3[ $key ].'::'. $tmplvar_arr_prval4[ $key ];
				}
			}
		break;
		...
		case 'colors-multiple': // scorn
			$tmplvar_arr_climg= $_POST["tv".$row['id']."climg"];
			$tmplvar_arr_clart= $_POST["tv".$row['id']."clart"];
			$tmplvar_arr_clnm= $_POST["tv".$row['id']."clnm"];
			if( ! empty( $tmplvar_arr_climg ) )
			{
				foreach( $tmplvar_arr_climg AS $key => $val )
				{
					$val= str_replace( "::", "", trim( $val ) );
					$val= str_replace( "||", "", $val );
					$tmplvar_arr_clart[ $key ]= str_replace( "::", "", trim( $tmplvar_arr_clart[ $key ] ) );
					$tmplvar_arr_clart[ $key ]= str_replace( "||", "", $tmplvar_arr_clart[ $key ] );
					$tmplvar_arr_clnm[ $key ]= str_replace( "::", "", trim( $tmplvar_arr_clnm[ $key ] ) );
					$tmplvar_arr_clnm[ $key ]= str_replace( "||", "", $tmplvar_arr_clnm[ $key ] );
					if( ! empty( $val ) ) $tmplvar .= ( ! empty( $tmplvar ) ? '||' : '' ) . $val .'::'. $tmplvar_arr_clart[ $key ] .'::'. $tmplvar_arr_clnm[ $key ];
				}
			}
		break;
